...
{
    ros::init(argc, argv, "waypoint_flight_node");
    ros::NodeHandle nh("~");

    /* ---------- 第1步：从参数服务器加载配置 ----------
     * waypoints.yaml 由 waypoint_flight.launch 中的 <rosparam> 标签加载到
     * /waypoint_flight/ 命名空间，这里用绝对路径读取。
     * 第三个参数为默认值，参数不存在时使用。
     */
    double flight_height, reach_threshold, hover_time;
    nh.param("/waypoint_flight/flight_height",   flight_height,   1.2);
    nh.param("/waypoint_flight/reach_threshold", reach_threshold, 0.3);
    nh.param("/waypoint_flight/hover_time",      hover_time,      3.0);

    std::vector<Waypoint> waypoints;
    XmlRpc::XmlRpcValue wp_list;
    if (nh.getParam("/waypoint_flight/waypoints", wp_list))
    {
